...
fun loadBusRoutes(context: Context, fileName: String): List<BusRoute> {
    ...
    return features.mapNotNull {
        ...
        if (type == "LineString") {
            ...
            val coordinates = coordinatesArray.mapNotNull { coordinateElement ->
                val coordinate = coordinateElement.asJsonArray
                val lon = coordinate[0].asDouble
                val lat = coordinate[1].asDouble
                GeoPoint(lat, lon)
            }
